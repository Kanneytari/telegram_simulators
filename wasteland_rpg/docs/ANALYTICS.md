...
- События только добавляются (`INSERT`) и не редактируются задним числом.
- `/reset` удаляет персонажа и прогресс, но **не удаляет историю аналитики**.
...
- `player_created` — первый персонаж игрока.
- `character_recreated` — персонаж создан заново после удаления старого состояния.
...
- `travel_advanced` — прохождение очередного участка; `value` = номер участка.
- `travel_finished` — прибытие в поселение.
