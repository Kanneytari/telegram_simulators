...
def build_workflow_allocation_router(game) -> Router:
    ...
    async def render(
        ...
        if not batch or not employee or batch["status"] != "warehouse":
            await present(
                target,
                "Партия или сотрудник уже недоступны.",
        ...
        global_rule = game.global_packaging_rule(player_id)
        mix = (
            f"×1 {global_rule['pct_1']}% · "
            f"×2 {global_rule['pct_2']}% · "
            f"×5 {global_rule['pct_5']}%"
        )
        retail_terms = game.compensation_policy(player_id, "courier")
        wholesale_quote = game.wholesale_handoff_quote(player_id, batch_id, quantity)

        risk = (
            f"🔴 Не покрыто депозитом после получения: <b>{unsecured:,} ₽</b>"
            if unsecured
            else "Объём полностью покрывается депозитом."
        )
        wholesale_text = ""
        if wholesale_quote:
            wholesale_text = (
                "\n\n<b>Оплата оптовику за эту передачу</b>\n"
                f"Основная комиссия: {wholesale_quote['base_amount']:,} ₽\n"
                f"Премия за непокрытый риск: {wholesale_quote['risk_amount']:,} ₽\n"
                f"Итого после успешной передачи: <b>{wholesale_quote['amount']:,} ₽</b>"
            )
        ...
        text = (
            ...
            f"{risk}\n\n"
            f"<b>Фасовки</b>\n{mix}\n\n"
            f"<b>Условия розницы</b>\n"
            f"{retail_terms['fixed_fee']:,} ₽ за успешный заказ + "
            f"{retail_terms['base_rate_bps'] / 100:.1f}% с продажи"
            f"{wholesale_text}"
        )

        presets = sorted({
            min(int(batch["remaining"]), value)
            for value in (5, 10, 25, 50, 100)
            if value <= int(batch["remaining"])
        })
        rows = []
        if presets:
            rows.append([
                InlineKeyboardButton(
                    text=str(value),
                    callback_data=f"workflow:alloc:{batch_id}:{employee_id}:{value}",
                )
        ...
        rows.append([
            InlineKeyboardButton(
                text="−10",
                callback_data=f"workflow:alloc:{batch_id}:{employee_id}:{max(1, quantity - 10)}",
            ),
            InlineKeyboardButton(
                text="−5",
                callback_data=f"workflow:alloc:{batch_id}:{employee_id}:{max(1, quantity - 5)}",
            ),
            InlineKeyboardButton(
                text="+5",
                callback_data=(
                    f"workflow:alloc:{batch_id}:{employee_id}:"
                    f"{min(int(batch['remaining']), quantity + 5)}"
                ),
            ),
            InlineKeyboardButton(
                text="+10",
                callback_data=(
                    f"workflow:alloc:{batch_id}:{employee_id}:"
                    f"{min(int(batch['remaining']), quantity + 10)}"
                ),
            ),
        ])
        rows.append([
        ...
        rows.append([
            InlineKeyboardButton(
                text="✅ Назначить",
                callback_data=f"workflow:allocconfirm:{batch_id}:{employee_id}:{quantity}",
            )
        ])
        rows.append([
            InlineKeyboardButton(
                text="← Партия",
                callback_data=f"workflow:batch:{batch_id}",
            )
        ])
        await present(target, text, InlineKeyboardMarkup(inline_keyboard=rows))

    @router.callback_query(
        F.data.startswith("workflow:alloc:")
        & ~F.data.startswith("workflow:allocconfirm:")
    )
