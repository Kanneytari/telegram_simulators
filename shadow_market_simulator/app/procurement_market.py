...
class ProcurementMarketSimulationEngine(StaffInsightSimulationEngine):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        with self.db.connect() as conn:
            ...
            self.db._ensure_column(conn, "supplier_offers", "offer_quality_mean", "REAL")
            self.db._ensure_column(conn, "supplier_offers", "offer_quality_sigma", "REAL")
            self.db._ensure_column(conn, "supplier_offers", "offer_reliability", "REAL")
            self.db._ensure_column(conn, "supplier_offers", "market_profile", "TEXT NOT NULL DEFAULT 'normal'")
    ...
    def ensure_player(self, player_id: int, username: str | None) -> bool:
        ...
        with self.db.connect() as conn:
            ...
            if not state:
                # Retire legacy offers generated by the older one-off system.
                conn.execute(
    ...
    def _maybe_refresh_offer(self, conn, player_id: int, now) -> None:
        # Disable the legacy "keep at least two offers" mechanic. The rotating
        # procurement market below owns offer supply completely.
        return None
    ...
    def refresh_procurement_market(self, player_id: int, now=None) -> int:
        ...
        with self.db.connect() as conn:
            ...
            # A long offline period need not create hundreds of writes; after several
            # partial rotations the current market is already effectively refreshed.
...
class ProcurementMarketGameService(StaffInsightGameService):
    ...
    def buy_offer_for_employee(self, player_id: int, offer_id: int, employee_id: int) -> str:
        ...
        # Buying the sole offer of a size must not leave a market segment empty.
