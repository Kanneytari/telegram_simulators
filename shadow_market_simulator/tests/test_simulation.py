...
def test_new_player_has_operational_state(tmp_path):
    ...
    with db.connect() as conn:
        assert conn.execute(
            "SELECT COUNT(*) FROM employees WHERE player_id=1001"
        ).fetchone()[0] >= 2
        assert conn.execute(
            "SELECT COUNT(*) FROM clients WHERE player_id=1001"
        ).fetchone()[0] >= 20
        assert conn.execute(
            "SELECT SUM(remaining) FROM batches WHERE player_id=1001"
        ).fetchone()[0] > 0
        assert conn.execute(
            "SELECT COUNT(*) FROM listings WHERE player_id=1001"
        ).fetchone()[0] == 21
...
def test_time_advance_without_retail_stock_creates_no_orders(tmp_path):
    ...
    with db.connect() as conn:
        conn.execute(
            "UPDATE shops SET last_simulated_at=? WHERE player_id=1001", (iso(before),)
        )
    ...
    with db.connect() as conn:
        orders = conn.execute(
            "SELECT COUNT(*) FROM orders WHERE player_id=1001"
        ).fetchone()[0]
        stock = conn.execute(
            "SELECT SUM(remaining) FROM batches WHERE player_id=1001"
        ).fetchone()[0]
