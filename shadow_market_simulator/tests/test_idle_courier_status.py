...
from app.staff_idle import IdleAwareGameService


def make_system(tmp_path):
    db = Database(str(tmp_path / "game.db"))
    db.init()
    simulation = ExpandedCatalogSimulationEngine(db, speed=1.0, rng=random.Random(71))
    simulation.seed_catalog()
    simulation.ensure_player(1001, "tester")
    game = IdleAwareGameService(db, simulation, rng=random.Random(72))
    return db, simulation, game


def clear_courier_work(db, courier_id: int) -> None:
    with db.connect() as conn:
        conn.execute("DELETE FROM employee_tasks WHERE player_id=1001 AND employee_id=?", (courier_id,))
        conn.execute("DELETE FROM retail_positions WHERE player_id=1001 AND employee_id=?", (courier_id,))
        conn.execute("DELETE FROM retail_allocations WHERE player_id=1001 AND retail_employee_id=?", (courier_id,))
        conn.execute(
            "UPDATE employees SET available=1, unavailable_until=NULL WHERE id=? AND player_id=1001",
            (courier_id,),
        )


def employee_view(game, employee_id: int):
    return next(row for row in game.employees(1001) if int(row["id"]) == employee_id)
...
def test_green_means_completely_idle_and_ready_for_new_delivery(tmp_path):
    ...
    with db.connect() as conn:
        ...
        batch = conn.execute(
            "SELECT * FROM batches WHERE player_id=1001 AND status='warehouse' AND remaining>0 ORDER BY id LIMIT 1"
        ).fetchone()
        warehouse = conn.execute(
            "SELECT id FROM employees WHERE player_id=1001 AND role='warehouse' AND active=1 ORDER BY id LIMIT 1"
        ).fetchone()
    ...
    _, recipients = game.retail_staff_for_batch(1001, int(batch["id"]))
...
def test_paused_courier_cannot_receive_new_assignment(tmp_path):
    ...
    with db.connect() as conn:
        ...
        batch = conn.execute(
            "SELECT id FROM batches WHERE player_id=1001 AND status='warehouse' AND remaining>0 ORDER BY id LIMIT 1"
        ).fetchone()
        conn.execute("UPDATE employees SET available=0 WHERE id=?", (courier["id"],))

    result = game.allocate_to_retail(1001, int(batch["id"]), int(courier["id"]), 5)
